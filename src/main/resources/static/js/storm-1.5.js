    ...
    template: function (json, template) {
      var temp = template
        ...
        for (; i < length; i++) {
          ...
          tem = match[i].split(' ');
          code += /^each/g.test(match[i]) ? ('$.each(' + tem[1] + ', function (' + (tem.length === 3 ? 'index' : tem[3]) + ', ' + tem[2] + ') {') : (/^if/g.test(match[i]) ? 'if (' + (match[i].split('if ')[1]) + ') {' : (/^elseif/g.test(match[i]) ? '} else if (' + (match[i].split('else if ')[1]) + ') {' : (/^else/g.test(match[i]) ? '} else {' : (/^\/each/g.test(match[i]) ? '});' : (/^\/(if)/g.test(match[i]) ? '}' : (/^var/g.test(match[i]) ? match[i] : ('r += ' + match[i] + ';')))))));
          code += '\n';
    ...
    validate: function (o) {
      var $this = this, // 表单
          ...
          defaults = { // 默认参数
            ...
            arrtribtue: ['required', 'minlength', 'pattern', 'firstspace', 'lastspace', 'spacechar', 'specialchar'], // 要验证的属性, 验证顺序
            customAttr: [], // 用户自定义验证
      ...
      _this.each = function () {
        ...
        $names.each(function () { // 开始遍历
          ...
          elem[name] = {
            ...
            firstspace: _this.defaultVal($elem.attr('firstspace')), // 首字符为空, 默认为true
            lastspace: _this.defaultVal($elem.attr('lastspace')), // 尾字符为空, 默认为true
            specialchar: _this.defaultVal($elem.attr('specialchar')) // 特殊字符, 默认为true
          };
